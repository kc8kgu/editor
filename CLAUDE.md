...
- Build and install on Windows: `./build.ps1` — installs deps, runs PyInstaller, and copies `editor.exe` to `%USERPROFILE%\.local\bin`
...
- **Selection**: `self.sel_anchor` holds the `(y, x)` anchor when a selection is active; `None` means no selection. Set by Shift+Arrow movement, and also by `search()` (which selects the matched text instead of tracking a separate highlight). `selection_range()` normalizes anchor/cursor into ordered `(start, end)`, used by `draw_line` (highlighting), `selected_text`, and `delete_selection`.
